...
Installer::Installer(const char* pkg, const char* target)
  ...
  if (::stat(target, &st) != 0) throw std::runtime_error("Invalid target path");
...
void Installer::installPackage() {
  ...
  // If "Distribution" file exists, then read it and find out choices enabled by
  // default (normally located in subdirectories) If no "Distribution" file
  // exists, then there are no subdirectories and we proceed with installation
  if (Reader* distr = m_xar->openFile("Distribution")) {
    ...
    std::cout << "installer: Package name is " << distribution->title()
              << std::endl;
    ...
    for (const DistributionXml::Choice& c : choices) {
      ...
      if (choiceTitle.empty()) choiceTitle = c.id;

      std::cout << "installer: Installing selected choice " << choiceTitle
                << std::endl;
...
std::shared_ptr<PackageInfoXml> Installer::loadPackageInfo() {
  ...
  if (!pkgInfoFile) throw std::runtime_error(path + " not found in .pkg");
...
std::string Installer::getInstallLocation() {
  ...
  if (installLocation != "/") path += installLocation;
  if (path[path.length() - 1] != '/') path += '/';
...
void Installer::extractPayload(const char* payloadFileName,
  ...
  if (destinationDir[destinationDir.length() - 1] != '/') destinationDir += '/';
  path = destinationDir;
  ...
  while (cpio->next(name, st, cpioFile)) {
    ...
    if (S_ISDIR(st.st_mode)) {
      ...
      std::cout << "installer: Warning: not installing a device node " << path
                << std::endl;
...
void Installer::extractFile(std::shared_ptr<Reader> reader, const char* dest) {
  ...
  while (done < length) {
    ...
    if (rd != toread) throw std::runtime_error("Short read in extractFile");
...
void Installer::runInstallStepScript(const char* scriptName) {
  ...
  if (!script.empty()) {
    exitCode = runScript(script.c_str(), m_scriptsDir.c_str(),
                         m_installLocation.c_str());
    if (exitCode != 0) throwScriptExitCode(exitCode);
  }
}
...
int Installer::installPayload(const char* subdir) {
  ...
  std::cout << "installer: Installing package " << identifier << " version "
            << m_pkgInfo->version() << " (" << m_pkgInfo->installKBytes()
            << " KB)\n";
  ...
  if (ReceiptsDb::getInstalledPackageInfo(m_pkgInfo->identifier().c_str(),
    ...
    // uninstall
    auto bom =
        ReceiptsDb::getInstalledPackageBOM(m_pkgInfo->identifier().c_str());
    if (bom) {
      uninstall(bom, installedPackageInfo.prefixPath.c_str());
    }
    isUpgrade = true;
  }
  ...
  if (scriptsDir[0] != 0) {
    if (m_verboseR)
      std::cout
          << "installer:PHASE:Running installer actions (preinstall)...\n";

    // run preinstall script
    runInstallStepScript(isUpgrade ? PackageInfoXml::SCRIPT_PREUPGRADE
                                   : PackageInfoXml::SCRIPT_PREINSTALL);
  }
  ...
  if (scriptsDir[0] != 0) {
    if (m_verboseR) {
      std::cout
          << "installer:PHASE:Running installer actions (postinstall)...\n";
      std::cout << "installer:%80.0\n";
    }

    // run postinstall script
    runInstallStepScript(isUpgrade ? PackageInfoXml::SCRIPT_POSTUPGRADE
                                   : PackageInfoXml::SCRIPT_POSTINSTALL);
  ...
  if (bomFile) extractFile(bomFile, bomPath.c_str());
...
int Installer::runScript(const char* scriptName, const char* scriptTempDir,
